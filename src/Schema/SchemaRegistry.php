...
class SchemaRegistry
{
    ...
    /**
     * Datasets directly reachable from $key by ONE foreign-key hop, in
     * EITHER direction.
     *
     * Both directions matter for NQ-001 (index-first prompting): a seed of
     * "customers" carries no relationship of its own — nothing points FROM
     * customers at anything — only "orders" does, by pointing AT customers.
     * A model naming only the dimension table must still pull in the fact
     * table, so the referencing side counts exactly as much as the
     * referenced side.
     *
     * Never returns a table with no schema file of its own: the same
     * allowsTable() gate `PromptBuilder::joinsFor()` already applies, so
     * every key this returns survives to a JOIN the validator will accept.
     *
    ...
    public function relatedDatasets(string $key): array
    {
        ...
        $tableName = $schema['tables']['primary']['name'] ?? null;
        $related = [];
        ...
        foreach ($this->all() as $otherKey => $otherSchema) {
            ...
            // Reverse direction: does $otherKey point AT $key's table?
            foreach ($otherSchema['tables']['primary']['relationships'] ?? [] as $rel) {
        ...
        foreach ($schema['tables']['primary']['relationships'] ?? [] as $rel) {
            $target = $rel['references_table'] ?? null;

            if (!$target || !$this->allowsTable($target)) {
                continue; // no schema file for the target — never advertise it
            }
            ...
            foreach ($this->all() as $otherKey => $otherSchema) {
                ...
                if ($this->sameTable($target, $otherSchema['tables']['primary']['name'] ?? null)) {
                    $related[$otherKey] = true;
                }
            }
        }
    ...
    /**
     * Walk the FK graph outward from $keys by up to $hops hops.
     *
     * This is the MECHANICAL half of index-first prompting (NQ-001): the
     * seed (model-named, routed, or carried over from a previous turn)
     * decides WHERE to start, and this decides what else the question
     * needs, without trusting the model to have named every table itself —
     * the argument for this design is exactly that an 8B model naming only
     * "customers" must still get "orders" back.
     *
     * @param array<int, string> $keys
     * @return array<int, string> $keys plus everything reached, deduplicated;
     *         unknown keys are silently dropped rather than expanded
     */
    public function expand(array $keys, int $hops = 1): array
    {
        $frontier = array_values(array_unique(array_filter($keys, fn ($key) => $this->has($key))));
        $seen = array_flip($frontier);

        for ($hop = 0; $hop < max(0, $hops); $hop++) {
            $next = [];
            ...
            if (empty($next)) {
                break; // nothing new reached — further hops would do nothing
            }
